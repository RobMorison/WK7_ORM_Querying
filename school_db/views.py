...
def problem_one(request):
  students_with_gpa_above_three = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')

  for student in students_with_gpa_above_three:
...
def problem_two(request):
  instructor_hired = Instructor.objects.filter(hire_date__year__lt = 2010).order_by('hire_date')

  for instructor in instructor_hired:
...
def problem_five(request):
  ...
  new_student = Student.objects.create(first_name = "Rob", last_name = "Morison", year = 12, gpa = 4.0)
